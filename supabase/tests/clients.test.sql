...
insert into public.clients (id, first_name, last_name, phone_e164, email) values
  ('00000000-0000-0000-0000-00000000c001', 'Zephyrine', 'Quillfeather-Testcase', '+23059990001', 'Priya.Test@Example.com'),
...
select results_eq(
  $$select id, confidence from public.find_similar_clients(p_first_name => 'Zephyrin', p_last_name => 'Quilfeather-Testcase')$$,
...
select is(
  (select count(*)::int from public.find_similar_clients('Zephyrine', 'Quillfeather-Testcase', '+23059990001', 'priya.test@example.com')),
  1, 'a client matching on several fields is listed once');
select is(
  (select confidence from public.find_similar_clients('Zephyrine', 'Quillfeather-Testcase', '+23059990001', null)),
